"""
Étend la météo au-delà de 2023 : récupère 2024-01-01 -> aujourd'hui-1
pour les 58 wilayas, puis fusionne avec weather_2015_2023.csv en une
table weather_2015_2026.csv unique.
"""
import datetime
import os
import time
...
CHECKPOINT_DIR = "../data/raw/weather_by_wilaya_2024_2026"
EXISTING_CSV = "../data/processed/weather_2015_2023.csv"
OUT_CSV = "../data/processed/weather_2015_2026.csv"
...
def main():
    ...
    if not frames:
        ...
        return
    new_weather = pd.concat(frames, ignore_index=True).rename(columns={"time": "date"})
    cols = ["wilaya_id", "wilaya_code", "date"] + DAILY_VARS
    new_weather = new_weather[cols]

    existing = pd.read_csv(EXISTING_CSV)
    combined = pd.concat([existing, new_weather], ignore_index=True)
    combined = combined.drop_duplicates(subset=["wilaya_id", "date"]).sort_values(["wilaya_id", "date"])
